...
const Exhibitions = () => {
  ...
  const allTags = [...new Set(
    exhibitions.flatMap(category => category.items.flatMap(item => item.tags || []))
  )];

  const exhibitionsToShow = exhibitions
    .find(cat => cat.category === activeTab)?.items
    .filter(ex => selectedTags.length === 0 || selectedTags.every(tag => ex.tags.includes(tag))) || [];

  const featuredExhibition = exhibitions
    .find(cat => cat.category === "current")?.items
    .find(ex => ex.featured);

  const ROOMS_COUNT = Math.ceil(exhibitionsToShow.length / 3);

  const navigateRoom = (dir) => {
    const next = currentRoom + (dir === 'next' ? 1 : -1);
    if (next >= 0 && next < ROOMS_COUNT) setCurrentRoom(next);
  };
